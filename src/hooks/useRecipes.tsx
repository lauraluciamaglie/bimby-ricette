...
import { Recipe, Course } from '@/types/recipe';
...
interface RecipesContextValue {
  ...
  /** Rimuove la ricetta: nasconde quella del ricettario, cancella quella creata dall'utente. */
  deleteRecipe: (id: string) => Promise<void>;
  /** Ripristina una ricetta del ricettario (annulla modifica/rimozione). */
  restoreRecipe: (id: string) => Promise<void>;
  /** Sposta la ricetta in un'altra categoria (crea/aggiorna una versione personale). */
  moveRecipe: (id: string, course: Course) => Promise<void>;
...
export function RecipesProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = useCallback(async () => {
    // 0. Ricette aggiunte/modificate dall'utente e ricette nascoste (sul dispositivo).
  ...
  const recipes = useMemo(() => {
    ...
    return [...onlyNew, ...merged].filter((r) => !hidden.has(r.id));
  }, [userRecipes, sourceRecipes, hiddenIds]);
  ...
  const deleteRecipe = useCallback(
    async (id: string) => {
      if (sourceRecipes.some((r) => r.id === id)) {
        setHiddenIds(await hideRecipe(id)); // ricetta del ricettario -> la nascondo
      } else {
        setUserRecipes(await deleteUserRecipe(id)); // ricetta creata da te -> la cancello
      }
    },
    [sourceRecipes]
  );

  const restoreRecipe = useCallback(async (id: string) => {
    setUserRecipes(await deleteUserRecipe(id)); // toglie l'eventuale versione personale
    setHiddenIds(await unhideRecipe(id)); // e la rende di nuovo visibile se era nascosta
  }, []);

  const moveRecipe = useCallback(
    async (id: string, course: Course) => {
      const base = userRecipes.find((r) => r.id === id) ?? sourceRecipes.find((r) => r.id === id);
      if (base) setUserRecipes(await addUserRecipe({ ...base, course }));
    },
    [userRecipes, sourceRecipes]
  );

  const value = useMemo(
    () => ({ recipes, loading, refreshing, source, refresh, getById, addRecipe, deleteRecipe, restoreRecipe, moveRecipe, isBuiltIn, isCustom }),
    [recipes, loading, refreshing, source, refresh, getById, addRecipe, deleteRecipe, restoreRecipe, moveRecipe, isBuiltIn, isCustom]
  );
